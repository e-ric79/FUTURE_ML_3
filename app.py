...
def load_models():
    ...
    # Load lightweight metadata instead of full CSV
    resumes_df = joblib.load('resumes_metadata.pkl')  # ['Category','extracted_skills']
...
def match_resumes(job_description, top_n=10):
    ...
    def calc_skill_match(candidate_skills_list):
        # Safely parse skills
        if isinstance(candidate_skills_list, str):
            try:
                candidate_skills = ast.literal_eval(candidate_skills_list)
            except:
                candidate_skills = []
        else:
            candidate_skills = candidate_skills_list or []
...
st.markdown('Automatically rank candidates based on job requirements using NLP and ML')
st.markdown('---')

# Sidebar
st.sidebar.title('About')
st.sidebar.info(
    'This system uses NLP to:\n\n'
...
if st.button('🔍 Find Best Candidates', type='primary', use_container_width=True):
    if job_desc and len(job_desc.strip()) > 20:
        ...
        for idx, (_, row) in enumerate(top_candidates.iterrows(), 1):
            with st.expander(f"**#{idx} - {row['Category']}** | Match: {row['final_score']:.1f}%"):
        ...
        st.warning('⚠️ Please enter a job description (at least 20 characters)')

# Footer
st.markdown('---')
st.caption('AI Resume Screener | Powered by NLP & ML | Lightweight metadata version')